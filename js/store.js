...
(function () {
  ...
  async function reserveSeat(day, seat, person) {
    ...
    /* 좌석표에 실리는 문서에는 '자리가 찼다' 는 사실만 담는다.
       ...
       읽는 logs 에만 있다. */
    /* 두 칸을 빈 값으로 보낸다. 값을 담지 않으니 새는 것은 없고,
       '이 필드가 문자열일 것' 을 요구하는 예전 보안 규칙도 그대로 통과한다.
       규칙과 화면 중 한쪽만 먼저 바뀌어도 예약이 멈추지 않게 하려는 것이다 —
       필드 하나가 어긋나 기록 쓰기가 통째로 거부됐던 일이 실제로 있었고,
       그것이 '반납이 아예 안 되던' 사고의 절반이었다. */
    const pub = { seat: Number(seat), nameMasked: '', sidHead: '' };
    const full = {
      seat: Number(seat),
